...
/****** Object:  View [NewPatient].[v_ov_zipcode_map]    Script Date: 9/5/2022 10:19:00 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE view [NewPatient].[v_ov_zipcode_map] as
...
select
	...
	,perc
	,case
		when perc < round((max(perc) over ())/3,2) then 'Low'
		when perc > (round((max(perc) over ())/3,2))*2 then 'High'
		else 'Medium'
		end as groupings
from(
	select distinct
		...
		,city_state
		,zipcode
		,count(full_name) over (partition by zipcode) as count
		,count(full_name) over () as area_count
		,round((convert(float,count(full_name) over (partition by zipcode))/
			convert(float,count(full_name) over ()))*100,2) as perc
	from(
		select
			a.appt_month
